...
Five modes share one helper:
  ...
  * ``(--images-dir DIR | --bundle DIR)
    --write-generated-provenance-template PATH`` —
    **generated-provenance sidecar template writer mode**: discovers
    the same flat PNG / JPG / JPEG folder operator mode uses (reuses
    the IG1..IG9 gate verbatim, either against ``--images-dir`` or
    against ``<bundle>/images`` when ``--bundle`` is supplied), then
    writes a starter JSON sidecar at PATH whose ``entries[]`` array
    carries one entry per discovered image, sorted by filename, with
    ``schema_version="1"`` and safe placeholder ``intent_summary`` /
    closed-enum ``placement_role`` / ``text_policy`` / ``subject_domain``
    fields that trivially pass GP1..GP13 when the operator places the
    file at ``<bundle>/generated_provenance.json``. ``generator_source``
    defaults to ``"operator_declared_generated"`` (the closed-set
    value added for this writer); ``"mock_generated"`` remains a
    legal hand-edited value, and anything else is refused at sidecar
    validation time. ``custom_descriptor`` is omitted by default and
    the operator may add one per entry matching
    ``^[a-z][a-z0-9_]{0,63}$`` before submission. The template path
    gate (GPT1..GPT6) mirrors ``--write-manifest-template`` (MT1..MT6)
    byte-for-byte: URI, symlink, symlink ancestor, REPO_ROOT anchor,
    missing / non-directory parent, and pre-existing target are all
    refused — stale bytes on a pre-existing target are preserved. The
    writer does NOT run the pipeline, does NOT produce a PPTX /
    workspace / summary / inventory / visual_quality / README, does
    NOT write anywhere under ``REPO_ROOT`` (GPT4 refuses the argument
    outright), and does NOT call D-One / MCP / Qoder / a public
    network / a model API / an image search / telemetry. When
    combined with ``--bundle`` the writer consumes ``<bundle>/images``
    only — any auto-detected ``<bundle>/manifest.json`` /
    ``<bundle>/generated_provenance.json`` is intentionally NOT
    forwarded so the writer's mutual-exclusion check and the GPT6
    pre-existing-target gate behave the same whether or not the
    bundle already carries those companion files.
...
or PPTX is created):
  ...
  GPT1..GPT6 (only when ``--write-generated-provenance-template`` is
        supplied) the sidecar-template path is not URI-shaped (GPT1),
        is not a symlink (GPT2), has no symlink ancestor (GPT3), does
        not lexically anchor under ``REPO_ROOT`` (GPT4), has an
        existing directory parent (GPT5), and does not already exist
        (GPT6 — the writer never overwrites operator files; stale
        bytes on a pre-existing target are preserved).
...
# Generator source enum is a small closed set today. The sidecar exists
# to describe synthetic / mock OR operator-declared generated bytes the
# operator already staged under <bundle>/images/. The helper itself
# does NOT call any generator — the enum names the operator's declared
# source, not a runtime touchpoint. ``mock_generated`` covers bytes
# materialised by the existing local mock pipeline;
# ``operator_declared_generated`` covers bytes the operator declares
# as generated outside this repo (e.g. a prior local image-gen run the
# operator did themselves) and is the value the
# ``--write-generated-provenance-template`` writer emits by default so
# operators do not need to hand-author the JSON.
...
# ---------------------------------------------------------------------------
# --write-generated-provenance-template path gate.
# ---------------------------------------------------------------------------


def _validate_generated_provenance_template_arg(
    template_path_str: str,
) -> tuple[Path | None, list[str]]:
    """Validate the ``--write-generated-provenance-template`` argument
    and return the resolved path. The caller MUST treat the argument as
    refused whenever ``failures`` is non-empty OR ``path`` is ``None``,
    and MUST NOT write to the path in that case (stale bytes on a
    pre-existing target are preserved).

    Mirrors the ``--write-manifest-template`` gate (MT1..MT6) under
    prefix GPT1..GPT6 so the two template-writer entry points carry
    one consistent contract — operators who already know the MT*
    failure modes see the same shapes here, and a future hardening of
    one gate trivially mirrors to the other.

      GPT1   URI-shaped argument (``file://`` / ``http://`` / ``data:`` /
             any RFC-3986 scheme prefix). The template writer accepts
             local paths only.
      GPT2   the path itself is a symlink (broken or resolvable).
             Silently following a symlink would let an attacker who
             controls the link target redirect operator writes.
      GPT3   any ancestor up to the filesystem root is a symlink.
             Closed system aliases like ``/tmp -> /private/tmp`` remain
             accepted via ``_forbidden_symlink_ancestor``.
      GPT4   the resolved path lexically anchors under ``REPO_ROOT``,
             compared BOTH case-sensitively AND on case-folded strings
             so a case-variant of the repo root still trips the gate on
             case-insensitive APFS / HFS+ / NTFS volumes (matches the
             MT4 reasoning).
      GPT5   the path's parent either does not exist or is not a
             directory. Refusing to ``mkdir -p`` avoids masking a typo
             in the operator's argument.
      GPT6   the path already exists (regular file, directory, or any
             other entry). The writer does not overwrite operator
             files; stale bytes on a pre-existing target are preserved.
    """
    if _has_uri_scheme(template_path_str):
        return None, [
            f"--write-generated-provenance-template argument "
            f"{template_path_str!r} looks URI-shaped (GPT1); only "
            f"local file paths are accepted."
        ]
    ...
    if template_path.is_symlink():
        try:
            ...
            tgt = "<unreadable>"
        return None, [
            f"--write-generated-provenance-template {template_path} is "
            f"a symlink (-> {tgt}) (GPT2); refused so a symlink target "
            f"cannot redirect operator writes."
        ]
    ...
    if forbidden_ancestor is not None:
        ancestor, tgt = forbidden_ancestor
        return None, [
            f"--write-generated-provenance-template {template_path} has "
            f"a symlink ancestor {ancestor} (-> {tgt}) (GPT3); refused "
            f"so a symlink in the operator's typed path cannot redirect "
            f"operator writes."
        ]
    ...
    try:
        ...
        return None, [
            f"--write-generated-provenance-template {template_path} "
    ...
    if under_root:
        return None, [
            f"--write-generated-provenance-template {resolved} "
            f"lexically anchors under REPO_ROOT={repo_root} (GPT4); "
            f"refused — generated-provenance templates must land "
            f"outside the committed repo tree."
        ]

    if not template_path.parent.exists():
        return None, [
            f"--write-generated-provenance-template {template_path} "
            f"parent {template_path.parent} does not exist (GPT5); "
            f"create the parent explicitly before re-running so a typo "
            f"cannot be masked by an implicit mkdir -p."
        ]
    if not template_path.parent.is_dir():
        return None, [
            f"--write-generated-provenance-template {template_path} "
            f"parent {template_path.parent} is not a directory (GPT5); "
            f"refused."
        ]
    ...
    if template_path.exists():
        return None, [
            f"--write-generated-provenance-template {template_path} "
            f"already exists (GPT6); refused to avoid overwriting an "
            f"operator file (stale bytes are preserved). Pass a fresh "
            f"path."
        ]
...
# Built-in default per-image generated-provenance sidecar template
# values. Centralised so the
# ``--write-generated-provenance-template`` writer emits a sidecar
# whose enum + free-text fields trivially pass GP1..GP13 when the
# operator places it at ``<bundle>/generated_provenance.json``. The
# placeholder strings are deliberately bland (no URL / URI / path
# separator / credential / public-share / confidential / AI-brand
# wording) so the safe-string scan GP10 accepts them verbatim.
_DEFAULT_SIDECAR_GENERATOR_SOURCE = "operator_declared_generated"
_DEFAULT_SIDECAR_PLACEMENT_ROLE = "local_region"
...
def _default_sidecar_intent_summary(image: _DiscoveredImage) -> str:
    """Safe placeholder ``intent_summary`` value the
    ``--write-generated-provenance-template`` writer emits for each
    discovered image.

    The placeholder is DELIBERATELY fixed — it does NOT interpolate
    ``image.operator_filename`` (or its asset_id). IG6 admits stems
    that match the schema id pattern (``^[A-Za-z0-9][A-Za-z0-9_.\\-]*$``)
    AND are at most 128 bytes, which is far more permissive than
    GP10's safe-string scan: stems like ``password`` /
    ``confidential`` / ``customer_data`` / ``apikey`` /
    ``twitter_logo`` / ``gpt4`` / ``claude-3-icon`` are all valid
    IG6 ids but contain substrings the GP10 deny lists (credential,
    confidential, upload-host, fake-success-brand) refuse. A
    placeholder that embeds the operator filename would
    self-generate a sidecar template that REFUSES on its own bytes
    when the operator copies it to
    ``<bundle>/generated_provenance.json`` — a contract regression
    Codex's stop-time review flagged. The fixed phrasing carries no
    URL / URI / path separator / credential / public-share /
    confidential / AI-brand wording so the template trivially passes
    GP10 on every valid IG6 filename; T102 round-trips the writer
    against ``password.png`` + ``confidential.jpg`` to lock the fix.
    The per-entry ``filename`` field still names each image so the
    operator can match each placeholder to its file by row order.
    """
    del image  # unused — see docstring for the rationale.
    return (
        "Operator-declared accent placeholder; "
        "edit per-entry intent before use (local-only)."
    )
...
# ---------------------------------------------------------------------------
# Generated-provenance sidecar template writer entrypoint.
# ---------------------------------------------------------------------------


def _run_generated_provenance_template_write_mode(
    *,
    images_dir_str: str,
    template_path_str: str,
    source_label: str,
) -> int:
    """Validate every operator argument, discover the images, and write
    a starter generated-image provenance sidecar template. Returns 0 on
    success, 2 on operator-input refusal (no filesystem mutation), 1 on
    write failure.
    ...
    and does NOT call D-One / MCP / Qoder / a public network / a model
    API / an image search / telemetry. It composes the same
    image-discovery gate (IG1..IG9) the normal operator mode applies
    plus the ``--write-generated-provenance-template`` path gate
    (GPT1..GPT6), then writes a deterministic JSON sidecar whose
    ``entries[]`` array carries one entry per discovered image, sorted
    by filename, with safe placeholder values that trivially pass the
    sidecar's GP1..GP13 gates when the operator places the file at
    ``<bundle>/generated_provenance.json``.

    ``source_label`` names the operator-typed argument that resolved to
    ``images_dir_str`` (either the literal ``--images-dir`` string the
    operator passed, or ``--bundle <bundle>/images`` when the bundle
    shortcut resolved it). It only flows into the OK-line diagnostic
    so the operator can trace which input the template was authored
    against.

    The generated file is byte-compatible with the optional
    ``<bundle>/generated_provenance.json`` sidecar shape: a re-run such
    as ``--bundle DIR --plan-out PATH`` (after the operator copies the
    template into ``DIR/generated_provenance.json``) accepts the
    template verbatim, the produced plan carries the top-level
    ``generated_provenance = {path, entry_count}`` block plus the
    per-row sidecar fields, and ``--bundle DIR --out-dir OUT
    --approved-plan PATH`` proceeds without drift.
    """
    images, _images_dir, image_failures = _validate_images_dir_arg(
        images_dir_str,
    )
    template_path, template_failures = (
        _validate_generated_provenance_template_arg(template_path_str)
    )
    ...
    body = {
        ...
        "entries": [
            {
                "filename": img.operator_filename,
                "generator_source": _DEFAULT_SIDECAR_GENERATOR_SOURCE,
                "intent_summary": _default_sidecar_intent_summary(img),
                "placement_role": _DEFAULT_SIDECAR_PLACEMENT_ROLE,
                "text_policy": _DEFAULT_SIDECAR_TEXT_POLICY,
                "subject_domain": _DEFAULT_SIDECAR_SUBJECT_DOMAIN,
            }
            for img in images
        ],
    }
    ...
    print(
        f"=== operator_local_images_to_editable_ppt "
        f"({source_label}, --write-generated-provenance-template "
        f"{template_path}) ==="
    )
    for img in images:
        print(
    ...
    try:
        ...
        print(
            f"FAIL: cannot write --write-generated-provenance-template "
            f"{template_path}: {type(exc).__name__}: {exc}",
            file=sys.stderr,
        )
        return 1
    ...
    if not template_path.is_file() or template_path.is_symlink():
        print(
            f"FAIL: expected --write-generated-provenance-template "
            f"{template_path} as a regular non-symlink file after "
            f"write",
            file=sys.stderr,
        )
        return 1
    ...
    print(
        f"OK (generated-provenance template): wrote {len(images)} "
        f"entr{'y' if len(images) == 1 else 'ies'} to {template_path}. "
        f"Edit the intent_summary / placement_role / text_policy / "
        f"subject_domain fields (and add an optional custom_descriptor "
        f"matching ^[a-z][a-z0-9_]{{0,63}}$ per entry if needed); "
        f"generator_source must stay in "
        f"{sorted(_SIDECAR_ALLOWED_GENERATOR_SOURCES)!r}. Copy the "
        f"file to <bundle>/generated_provenance.json and re-run with "
        f"--bundle DIR + --out-dir OUT (or --plan-out PATH). "
        f"Local-only — does NOT call D-One, MCP, Qoder, a public "
        f"network, a model API, an image search, or telemetry."
    )
    return 0
...
def _run_self_tests() -> int:
    ...
    # ----- T96..T102: --write-generated-provenance-template mode -----
    #
    # The writer's purpose is to let an operator drop a starter
    # generated-image sidecar into a bundle without hand-authoring
    # JSON. The seven probes below cover the spec's "must prove" list
    # plus a Codex-flagged self-generated-sidecar regression:
    #
    #   * T96 template happy path against --images-dir;
    #   * T97 template happy path against --bundle (the operator
    #     handoff shape) — the writer must accept both inputs;
    #   * T98 the written template, copied verbatim to
    #     <bundle>/generated_provenance.json, validates against
    #     GP1..GP13 AND the resulting plan-out carries the per-row
    #     sidecar fields plus the top-level generated_provenance
    #     {path, entry_count} block;
    #   * T99 the same template-driven sidecar drives a full
    #     --approved-plan loop end to end (--plan-out, then a
    #     follow-up --out-dir --approved-plan run on the same bundle
    #     succeeds with the locked summary block);
    #   * T100 a hand-edited template carrying an unknown
    #     generator_source value (anything off the closed set) refuses
    #     at the GP9 sidecar gate so a tampered template cannot smuggle
    #     a new source through;
    #   * T101 the writer's path gate (GPT1..GPT6) refuses every
    #     documented bad-output-path shape AND leaves zero filesystem
    #     state behind on refusal;
    #   * T102 self-generated-sidecar round-trip — the writer's
    #     fixed-placeholder intent_summary must accept any valid IG6
    #     filename whose stem happens to contain a GP10-denied
    #     substring (password / confidential / gpt4 / etc.), so the
    #     emitted template never refuses on its own bytes when the
    #     operator copies it to <bundle>/generated_provenance.json.

    # T96 happy path: write a generated-provenance sidecar template
    # from --images-dir into a tempdir outside the repo. Asserts the
    # template has the documented shape (schema_version="1", one
    # entries[] item per discovered image sorted by filename, every
    # required field populated with the helper's safe defaults,
    # no custom_descriptor) AND that the writer produced nothing
    # besides the template under its parent directory.
    with tempfile.TemporaryDirectory(prefix="op-helper-T96-") as raw_td:
        td = Path(raw_td)
        images_dir = td / "images"
        template_path = td / "generated_provenance_template.json"
        _write_synthetic_images(images_dir)
        rc_t = _run_generated_provenance_template_write_mode(
            images_dir_str=str(images_dir),
            template_path_str=str(template_path),
            source_label=f"--images-dir {images_dir}",
        )
        ok = rc_t == 0 and template_path.is_file()
        detail = ""
        body: dict | None = None
        if not ok:
            ...
            try:
                ...
                if body.get("schema_version") != "1":
                    ...
                    detail = (
                        f"schema_version="
                        f"{body.get('schema_version')!r}; expected '1'"
                    )
                elif not isinstance(body.get("entries"), list):
                    ok = False
                    detail = (
                        f"entries is "
                        f"{type(body.get('entries')).__name__}; "
                        f"expected list"
                    )
                elif len(body["entries"]) != 2:
                    ok = False
                    detail = (
                        f"entries has {len(body['entries'])} items; "
                        f"expected 2"
                    )
                else:
                    expected_filenames = [
                    ...
                    actual_filenames = [
                        e.get("filename") for e in body["entries"]
                    ]
                    if actual_filenames != expected_filenames:
                        ok = False
                        detail = (
                            f"filenames {actual_filenames!r}; "
                            f"expected sorted "
                            f"{expected_filenames!r}"
                        )
                    else:
                        required_keys = {
                            "filename", "generator_source",
                            "intent_summary", "placement_role",
                        ...
                        for i, entry in enumerate(body["entries"]):
                            ...
                            if entry_keys != required_keys:
                                ...
                                detail = (
                                    f"entries[{i}] keys "
                                    f"{sorted(entry_keys)!r}; expected "
                                    f"exactly {sorted(required_keys)!r} "
                                    f"(custom_descriptor must be "
                                    f"omitted by default)"
                                )
                                break
                            if (
                                entry["generator_source"]
                                != _DEFAULT_SIDECAR_GENERATOR_SOURCE
                            ):
                                ok = False
                                detail = (
                                    f"entries[{i}].generator_source="
                                    f"{entry['generator_source']!r}; "
                                    f"expected "
                                    f"{_DEFAULT_SIDECAR_GENERATOR_SOURCE!r}"
                                )
                                break
                            if (
                                entry["placement_role"]
                                != _DEFAULT_SIDECAR_PLACEMENT_ROLE
                                or entry["text_policy"]
                                != _DEFAULT_SIDECAR_TEXT_POLICY
                                or entry["subject_domain"]
                                != _DEFAULT_SIDECAR_SUBJECT_DOMAIN
                            ):
                                ok = False
                                detail = (
                                    f"entries[{i}] enum drift: "
                                    f"{entry!r}"
                                )
                                break
                            intent = entry["intent_summary"]
                            if (
                                not isinstance(intent, str)
                                or not intent
                                or len(intent)
                                > _SIDECAR_INTENT_SUMMARY_MAX_LEN
                            ):
                                ok = False
                                detail = (
                                    f"entries[{i}].intent_summary="
                                    f"{intent!r}"
                                )
                                break
        siblings = sorted(
        ...
        if ok and siblings != ["generated_provenance_template.json"]:
            ok = False
            detail = (
                f"writer produced extra siblings under td: "
                f"{siblings!r}"
            )
        results.append(_ProbeResult(
            "T96 --write-generated-provenance-template happy path "
            "against --images-dir: writes a schema-valid sidecar "
            "template with safe defaults and produces no other "
            "filesystem state",
            ok, detail,
        ))

    # T97 happy path against --bundle: the operator-handoff shape must
    # work for the new writer the same way --images-dir does. Asserts
    # rc=0 + the same body-shape invariants T96 locked, plus that the
    # writer ignored any auto-detected <bundle>/manifest.json /
    # <bundle>/generated_provenance.json in the bundle (the bundle in
    # this probe deliberately carries neither — the gate check happens
    # via the per-bundle code path).
    with tempfile.TemporaryDirectory(prefix="op-helper-T97-") as raw_td:
        td = Path(raw_td)
        bundle = td / "bundle"
        bundle.mkdir()
        _write_synthetic_images(bundle / "images")
        template_path = td / "tpl.json"
        rc = main([
            "--bundle", str(bundle),
            "--write-generated-provenance-template",
            str(template_path),
        ])
        ok = rc == 0 and template_path.is_file()
        detail = f"rc={rc}, template_exists={template_path.is_file()}"
        if ok:
            try:
                body = json.loads(template_path.read_text())
            except (OSError, json.JSONDecodeError) as exc:
                ok = False
                detail = f"cannot parse {template_path}: {exc}"
            else:
                actual_filenames = [
                    e.get("filename")
                ...
                if actual_filenames != [
                    ...
                    ok = False
                    detail = (
                        f"filenames {actual_filenames!r}; expected "
                        f"sorted ['alpha_marker.png', 'beta_marker.jpg']"
                    )
        # Ensure NO review-package artifact materialised under the
        # bundle (the writer is not allowed to run the pipeline).
        if ok and (bundle / "generated_provenance.json").exists():
            ok = False
            detail = (
                "writer leaked a sidecar into <bundle>/ — must write "
                "ONLY to the supplied template path"
            )
        results.append(_ProbeResult(
            "T97 --bundle + --write-generated-provenance-template "
            "happy path: writes the template, ignores any "
            "auto-detected bundle companion files, and does not "
            "touch the bundle itself",
            ok, detail,
        ))

    # T98 template validates at <bundle>/generated_provenance.json:
    # after the writer emits the template, copying it verbatim to the
    # bundle's expected sidecar path must let the existing sidecar
    # gate (GP1..GP13) accept it. Drives the full --bundle --plan-out
    # path so the helper's own validator is the source of truth.
    # Asserts the resulting plan carries:
    #   * top-level generated_provenance = {path: <sidecar>,
    #     entry_count: 2};
    #   * every per-image row's sidecar fields (generator_source,
    #     intent_summary, placement_role, text_policy, subject_domain).
    with tempfile.TemporaryDirectory(prefix="op-helper-T98-") as raw_td:
        td = Path(raw_td)
        bundle = td / "bundle"
        bundle.mkdir()
        _write_synthetic_images(bundle / "images")
        ...
        ok = rc_w == 0 and template_path.is_file()
        detail = f"writer rc={rc_w}"
        sidecar_path: Path | None = None
        if ok:
            sidecar_path = bundle / "generated_provenance.json"
            shutil.copyfile(template_path, sidecar_path)
            plan_out = td / "plan.json"
            rc_p = main([
                "--bundle", str(bundle),
                "--plan-out", str(plan_out),
            ])
            if rc_p != 0 or not plan_out.is_file():
                ok = False
                detail = (
                    f"plan rc={rc_p}, plan_exists={plan_out.is_file()}"
                )
            else:
                try:
                    ...
                    gp = plan.get("generated_provenance")
                    if not isinstance(gp, dict):
                        ok = False
                        detail = (
                            f"plan.generated_provenance={gp!r}; "
                            f"expected dict"
                        )
                    elif gp.get("path") != str(sidecar_path):
                        ok = False
                        detail = (
                            f"plan.generated_provenance.path="
                            f"{gp.get('path')!r}; expected "
                            f"{str(sidecar_path)!r}"
                        )
                    elif gp.get("entry_count") != 2:
                        ok = False
                        detail = (
                            f"plan.generated_provenance.entry_count="
                            f"{gp.get('entry_count')!r}; expected 2"
                        )
                    else:
                        for i, row in enumerate(plan.get("images") or []):
                            missing = [
                                k for k in (
                                    "generator_source",
                                    "intent_summary",
                            ...
                            if missing:
                                ok = False
                                detail = (
                                    f"plan.images[{i}] missing sidecar "
                                    f"fields {missing!r}"
                                )
                                break
                            if (
                                ...
                                ok = False
                                detail = (
                                    f"plan.images[{i}].generator_source="
                                    f"{row['generator_source']!r}"
                                )
                                break
        results.append(_ProbeResult(
            "T98 written template validates at "
            "<bundle>/generated_provenance.json (GP1..GP13) and "
            "--plan-out surfaces the top-level generated_provenance "
            "block plus the per-row sidecar fields",
            ok, detail,
        ))

    # T99 approved-plan loop end-to-end: write the template, place it
    # into the bundle, run --plan-out, then run --out-dir +
    # --approved-plan against the same bundle. The follow-up run must
    # succeed (rc=0) AND the produced summary must carry
    # generated_provenance={path, entry_count} AND
    # image_provenance[*] rows must carry the sidecar fields. Locks
    # the spec's "approved-plan run succeeds" claim.
    with tempfile.TemporaryDirectory(prefix="op-helper-T99-") as raw_td:
        td = Path(raw_td)
        bundle = td / "bundle"
        bundle.mkdir()
        _write_synthetic_images(bundle / "images")
        ...
        if ok:
            ...
            plan_out = td / "approved_plan.json"
            rc_p = main([
                "--bundle", str(bundle),
                "--plan-out", str(plan_out),
            ])
            if rc_p != 0 or not plan_out.is_file():
                ...
                if rc_r != 0:
                    ok = False
                    detail = f"approved-plan run rc={rc_r}"
                else:
                    try:
                        summary = json.loads(
                            (out_dir / "summary.json").read_text(),
                        ...
                        ok = False
                        detail = f"cannot parse summary: {exc}"
                    else:
                        gp = summary.get("generated_provenance")
                        if not isinstance(gp, dict):
                            ok = False
                            detail = (
                                f"summary.generated_provenance={gp!r}; "
                                f"expected dict on a sidecar-carrying "
                                f"approved-plan run"
                            )
                        elif (
                            summary.get("approved_plan", {})
                            .get("matched")
                            is not True
                        ):
                            ok = False
                            detail = (
                                ...
                                f"{summary.get('approved_plan')!r}"
                            )
                        else:
                            for i, p in enumerate(
                                summary.get("image_provenance") or []
                            ):
                                if (
                                    p.get("generator_source")
                                    != _DEFAULT_SIDECAR_GENERATOR_SOURCE
                                ):
                                    ok = False
                                    detail = (
                                        ...
                                        f"generator_source="
                                        f"{p.get('generator_source')!r}"
                                    )
                                    break
        results.append(_ProbeResult(
            "T99 template-driven approved-plan loop end-to-end: "
            "writer emits template; copying to bundle's sidecar path "
            "passes --plan-out + --out-dir --approved-plan with "
            "summary.generated_provenance carrying {path, "
            "entry_count} and image_provenance rows carrying "
            "sidecar fields",
            ok, detail,
        ))

    # T100 hand-edited template with an unknown generator_source must
    # refuse at the sidecar gate (GP9) when placed at
    # <bundle>/generated_provenance.json. Locks the "reject anything
    # else" half of the spec — the new operator_declared_generated
    # value is allowed, the legacy mock_generated value is allowed,
    # but anything off the closed set is refused before any pipeline
    # subprocess or out-dir artifact materialises.
    with tempfile.TemporaryDirectory(prefix="op-helper-T100-") as raw_td:
        td = Path(raw_td)
        bundle = td / "bundle"
        bundle.mkdir()
        _write_synthetic_images(bundle / "images")
        ...
        if ok:
            body = json.loads(template_path.read_text())
            # Hand-tamper one entry to carry a value outside the
            # closed generator_source set. The sidecar validator's
            # GP9 must refuse on placement.
            body["entries"][0]["generator_source"] = "operator_unknown"
            ...
            if not ok:
                detail = (
                    f"run rc={rc_r}, out_exists={out_dir.exists()}"
                )
        results.append(_ProbeResult(
            "T100 hand-edited template with unknown generator_source "
            "(off the closed set {mock_generated, "
            "operator_declared_generated}) is refused with rc=2 by "
            "the sidecar GP9 gate; no out-dir artifact is created",
            ok, detail,
        ))

    # T101 GPT1..GPT6 path gates: bad output paths leave zero
    # filesystem state behind. Covers URI-shaped (GPT1), REPO_ROOT
    # anchor (GPT4), pre-existing target (GPT6), symlink target (GPT2),
    # missing parent (GPT5). Each refusal goes through
    # ``_run_generated_provenance_template_write_mode`` directly so
    # the diagnostic side of the gate is exercised independent of
    # the bundle resolver.
    with tempfile.TemporaryDirectory(prefix="op-helper-T101-") as raw_td:
        td = Path(raw_td)
        images_dir = td / "images"
        _write_synthetic_images(images_dir)
        results_t101: list[tuple[str, bool, str]] = []

        # GPT1 URI-shaped.
        uri_path = (
            "file:///tmp/op_helper_t101_gpt_template_must_not_exist.json"
        )
        rc1 = _run_generated_provenance_template_write_mode(
            images_dir_str=str(images_dir),
            template_path_str=uri_path,
            source_label=f"--images-dir {images_dir}",
        )
        _, fails1 = _validate_generated_provenance_template_arg(uri_path)
        results_t101.append((
            "GPT1 URI-shaped",
            (
                rc1 == 2
                and any("GPT1" in f for f in (fails1 or []))
                and not Path(uri_path).exists()
            ),
            f"rc={rc1}, fails={fails1!r}",
        ))

        # GPT4 inside REPO_ROOT.
        bad = REPO_ROOT / "operator_gp_template_should_not_land_here.json"
        rc4 = _run_generated_provenance_template_write_mode(
            images_dir_str=str(images_dir),
            template_path_str=str(bad),
            source_label=f"--images-dir {images_dir}",
        )
        results_t101.append((
            "GPT4 inside-REPO_ROOT",
            rc4 == 2 and not bad.exists(),
            f"rc={rc4}, leaked={bad.exists()}",
        ))

        # GPT5 parent missing.
        bad_parent = td / "no_such_dir" / "tpl.json"
        rc5 = _run_generated_provenance_template_write_mode(
            images_dir_str=str(images_dir),
            template_path_str=str(bad_parent),
            source_label=f"--images-dir {images_dir}",
        )
        results_t101.append((
            "GPT5 missing parent",
            rc5 == 2 and not bad_parent.exists(),
            f"rc={rc5}, leaked={bad_parent.exists()}",
        ))

        # GPT6 pre-existing target with stale bytes preserved.
        existing = td / "stale.json"
        existing.write_text(
            "OPERATOR-STALE-BYTES",
            encoding="utf-8",
        )
        rc6 = _run_generated_provenance_template_write_mode(
            images_dir_str=str(images_dir),
            template_path_str=str(existing),
            source_label=f"--images-dir {images_dir}",
        )
        results_t101.append((
            "GPT6 pre-existing target",
            (
                rc6 == 2
                and existing.is_file()
                and existing.read_text(encoding="utf-8")
        ...
        # GPT2 symlink target (broken).
        link_target = td / "symlink_target.json"
        link_target.symlink_to(td / "nonexistent")
        rc2 = _run_generated_provenance_template_write_mode(
            images_dir_str=str(images_dir),
            template_path_str=str(link_target),
            source_label=f"--images-dir {images_dir}",
        )
        results_t101.append((
            "GPT2 symlink target",
            rc2 == 2 and link_target.is_symlink(),
            f"rc={rc2}",
        ))

        ok = all(r[1] for r in results_t101)
        detail = "; ".join(
            f"{name}: {ok_b}/{d}"
            for name, ok_b, d in results_t101
            if not ok_b
        )
        results.append(_ProbeResult(
            "T101 GPT1/GPT2/GPT4/GPT5/GPT6 path-gate refusals: bad "
            "output paths refuse with rc=2 and leave zero filesystem "
            "state behind (stale bytes on a pre-existing target are "
            "preserved)",
            ok, detail,
        ))

    # T102 self-generated-sidecar round-trip with tricky filenames.
    # IG6 admits stems that match ``^[A-Za-z0-9][A-Za-z0-9_.\-]*$`` —
    # far more permissive than GP10's safe-string scan, which refuses
    # substrings like ``password`` (credential), ``confidential``
    # (confidential marker), ``twitter`` (public-host channel),
    # ``gpt4`` / ``claude-3`` (fake-success AI brand), and many more.
    # An early version of the writer interpolated
    # ``image.operator_filename`` into the per-entry placeholder, so
    # a file named ``password.png`` produced an
    # ``intent_summary = "Operator-declared accent placeholder for
    # 'password.png' (edit before use; local-only)."`` template that
    # REFUSED on its own bytes when copied to
    # ``<bundle>/generated_provenance.json`` — the writer
    # self-generated an invalid sidecar. Codex's stop-time review
    # flagged this. The fix uses a fixed placeholder that does NOT
    # depend on the filename; this probe drives the writer against
    # two such tricky filenames and asserts the produced template
    # passes GP1..GP13 byte-identically. It calls the sidecar
    # validator directly so the probe does not depend on the bundle
    # wiring or any pipeline subprocess.
    with tempfile.TemporaryDirectory(prefix="op-helper-T102-") as raw_td:
        td = Path(raw_td)
        images_dir = td / "images"
        images_dir.mkdir()
        (images_dir / "password.png").write_bytes(_TINY_PNG_BYTES)
        (images_dir / "confidential.jpg").write_bytes(_TINY_JPEG_BYTES)
        template_path = td / "tpl.json"
        rc_w = _run_generated_provenance_template_write_mode(
            images_dir_str=str(images_dir),
            template_path_str=str(template_path),
            source_label=f"--images-dir {images_dir}",
        )
        ...
        detail = f"writer rc={rc_w}"
        if ok:
            entries, _resolved, fails = (
                _validate_generated_provenance_sidecar(
                    str(template_path),
                    ["confidential.jpg", "password.png"],
                )
            )
            if fails or entries is None or len(entries) != 2:
                ok = False
                detail = (
                    f"sidecar fails={fails!r}, "
                    f"entries={entries!r}"
                )
        results.append(_ProbeResult(
            "T102 self-generated-sidecar round-trip: writer-emitted "
            "template for valid IG6 filenames whose stems contain "
            "GP10-denied substrings (password / confidential) passes "
            "GP1..GP13 sidecar validation byte-identically — locks "
            "the fixed-placeholder contract so a future regression "
            "that interpolates the filename back into intent_summary "
            "fails closed on this probe",
            ok, detail,
        ))
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        ...
        help=(
            "Generated-provenance sidecar template writer mode. When "
            "supplied, the helper discovers the images using the same "
            "IG1..IG9 gates (against --images-dir, or against "
            "<bundle>/images when --bundle is supplied), then writes a "
            "starter JSON sidecar at PATH whose entries[] array carries "
            "one entry per discovered image (sorted by filename) with "
            "safe placeholder intent_summary / placement_role / "
            "text_policy / subject_domain values that trivially pass "
            "GP1..GP13 when the operator places the file at "
            "<bundle>/generated_provenance.json. generator_source "
            f"defaults to {_DEFAULT_SIDECAR_GENERATOR_SOURCE!r} (the "
            f"operator-declared variant added for this writer; the "
            f"validator still accepts the existing 'mock_generated' "
            f"value, and anything else is refused). The template path "
            "gate (GPT1..GPT6) mirrors --write-manifest-template: must "
            "not be URI-shaped, a symlink, have a symlink ancestor, "
            "anchor under the repo, point at a missing or "
            "non-directory parent, or already exist (stale bytes on a "
            "pre-existing target are preserved). The writer does NOT "
            "run the pipeline, does NOT produce a PPTX / workspace / "
            "summary / inventory / visual_quality report / README, and "
            "does NOT call D-One / MCP / Qoder / a public network / a "
            "model API / an image search / telemetry. Mutually "
            "exclusive with --out-dir / --manifest / --plan-out / "
            "--approved-plan / --write-manifest-template / --self-test; "
            "requires --images-dir OR --bundle."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "symlink ancestor, be missing, or be a regular file. May "
            "combine with --write-generated-provenance-template, in "
            "which case the writer consumes <bundle>/images only and "
            "ignores any auto-detected <bundle>/manifest.json / "
            "<bundle>/generated_provenance.json. Mutually exclusive "
            "with --images-dir / --manifest / --write-manifest-template "
            "/ --self-test."
        ),
    )
    parser.add_argument(
    ...
    if args.self_test:
        if (
            ...
            or args.write_generated_provenance_template is not None
    ...
    if args.bundle is not None:
        ...
        # bundle-only by design. When the operator combined --bundle
        # with --write-generated-provenance-template, the writer only
        # consumes the images dir to enumerate filenames for the new
        # template; any auto-detected bundle ``manifest.json`` /
        # ``generated_provenance.json`` is intentionally NOT forwarded
        # so the writer's mutual-exclusion check is not surprised by
        # files the operator did not type, AND so a bundle that
        # already carries a sidecar can still produce a sibling
        # template at a fresh --write-generated-provenance-template
        # path without colliding via GPT6.
        args.images_dir = resolved_images_dir
        if args.write_generated_provenance_template is None:
    ...
    if args.write_manifest_template is not None:
        ...
        if (
            ...
            print(
                ...
                "/ --write-generated-provenance-template (the template "
    ...
    if args.write_generated_provenance_template is not None:
        if args.images_dir is None:
            print(
                "FAIL: --write-generated-provenance-template requires "
                "--images-dir (or --bundle).",
                file=sys.stderr,
            )
            return 2
        if (
            ...
            print(
                "FAIL: --write-generated-provenance-template does not "
                "take --out-dir / --manifest / --plan-out / "
                "--approved-plan (the sidecar-template writer does not "
                "run the pipeline; the produced file is metadata only).",
                file=sys.stderr,
            )
            return 2
        # Surface which operator-typed argument resolved to the images
        # dir so the OK-line diagnostic names the source the writer
        # walked. ``--bundle`` callers see the resolved
        # ``<bundle>/images`` path; explicit ``--images-dir`` callers
        # see their typed string verbatim.
        source_label = (
            f"--bundle {args.bundle} (--images-dir {args.images_dir})"
            if args.bundle is not None
            else f"--images-dir {args.images_dir}"
        )
        return _run_generated_provenance_template_write_mode(
            images_dir_str=args.images_dir,
            template_path_str=(
                args.write_generated_provenance_template
            ),
    ...
    if missing:
        print(
            ...
            f"--write-generated-provenance-template PATH to write a "
            f"starter generated-provenance sidecar without running "
            f"the pipeline, --plan-out PATH to write a preflight plan "
            f"without running the pipeline, or --bundle DIR as the "
            f"operator-handoff shortcut for --images-dir / "
            f"--manifest).",
            file=sys.stderr,
        )
        return 2
