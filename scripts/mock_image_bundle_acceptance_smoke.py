...
``--self-test`` also runs seven tempfixture fail-closed probes; each
...
def _run_self_test() -> int:
    ...
    print(
        ...
        "byte-identical on the produced plan); and "
        "scripts/__pycache__/ is byte-identical even without "
        "PYTHONDONTWRITEBYTECODE in the subprocess env. All seven "
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "marker; scripts/__pycache__/ is byte-identical before and "
            "after the runner subprocess EVEN when "
            "PYTHONDONTWRITEBYTECODE is stripped from the subprocess "
            "env; and no committed repo path (examples/ or scripts/) "
            "is mutated. --self-test additionally runs seven "
            "tempfixture fail-closed probes (missing bundle, "
            "symlinked bundle parent, parent-traversal `..` bundle "
            "path, URI-shaped image_manifest_spec local_path, "
            "traversal `../` image_manifest_spec local_path, wrong "
            "placement_role flipping cover_accent to local_region "
            "while the prompt still carries overlay-reservation cues, "
            "direct probe of _bundle_placement_roles against a bundle "
            "copy missing the local_region request) and asserts each "
            "aborts the runner with no PPTX produced (or — for the "
            "direct helper probe — surfaces the regression at the "
            "static-evidence layer). MOCK / STUB only — NOT real "
            "D-One integration; no MCP, no public network, no model "
            "API, no image search, no Qoder, no telemetry, no "
            "external service. Stdlib-only."
        ),
    )
    parser.add_argument(
        "--self-test", action="store_true",
        help=(
            "Run the happy path + seven tempfixture negative probes "
