...
The project should meet the same business need as a PPT-generation workflow. Upstream `ppt-master` may be used as a direction radar per `references/agent-operating-context.md` — especially for high-level image-generation and image-asset ideas — but no code, prompts, docs, templates, examples, assets, or wording may be copied, paraphrased, ported, imported, or summarized into this repo.
...
Before implementation work, read `references/agent-operating-context.md` for the durable project direction, current first-stage target, and current-phase scope.
...
`image_slot` referencing a PNG / JPG / JPEG asset is exported as a native PPTX `<p:pic>` whose `<a:blip r:embed>` points at the bytes copied into `ppt/media/imageN.<ext>` (registered with the matching `image/png` or `image/jpeg` content-type Default and an internal `image` relationship); an `image_slot` with no embeddable asset — including SVG / GIF / WebP, which are not yet embeddable — falls back to a native placeholder shape carrying the `image_manifest` alt_text, and that broader-format embedding remains TODO. Anything outside the supported subset must fail closed: `chart_placeholder` primitives, layouts outside the per-stage allow-lists above, malformed render_models, undeclared image refs, and unsafe manifest paths all abort the run with a clear per-slide error rather than producing a partial deck.
...
The PPTX contract validator (`scripts/validate_pptx_contract.py`) now enforces `relationships.allow_list` over the six canonical OOXML rel types the exporter emits today (`officeDocument`, `slide`, `slideMaster`, `slideLayout`, `theme`, and the per-slide `image` rel for embedded PNG / JPG / JPEG media), the embedded-media gates `media.targets_internal` / `media.inventory` / `media.embedded_only` (every `image` target is an internal `ppt/media/` part in the {png, jpg, jpeg} allow-list with no orphan media, and no `<a:blip r:link>` external image appears anywhere), and `minimal_evidence.every_slide_has_native_shape` alongside the earlier container, package, and minimal-evidence gates. A caller can also pass `--expected-slide-count N` to activate the `slide_count.expected` gate (the slide-part count must equal `N` or the run fails closed — catches an exporter that silently truncated the deck). Full PPTX coverage (every layout, every primitive, full editability inventory, theme palette mapping, determinism inventory, layout/primitive-scope inspection of the produced PPTX) remains **not** implemented; chart rendering, visual regression, D-One integration, Qoder CLI integration, arbitrary-SVG parsing, and full-slide raster fallback also remain TODO. SVG repair (clipping out-of-bounds shapes, font fallback) and glyph-level text-overflow detection are still TODO.
