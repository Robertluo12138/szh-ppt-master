...
- **no escape hatch**: no `<foreignObject>` may appear anywhere in the tree. The element-name comparison is **case-folded**: every variant — `<foreignobject>`, `<FOREIGNOBJECT>`, `<ForeignObject>`, `<ForEignObJecT>` — is detected, because the HTML5 parser's SVG element-name adjustment table case-corrects each variant back to canonical camelCase before the inline SVG renders. A case-sensitive validator would otherwise miss the bypass while the browser still rendered any `<script>` the foreign object hosts. The geometry walk and the reference-attribute walk apply the **same case-fold rule to BOTH element tags AND attribute keys** — `<RECT X="3000" Y="3000" WIDTH="5000" HEIGHT="5000"/>` still trips the out-of-canvas gate (HTML5 lowercases the attribute keys too, so a case-sensitive `attrib["x"]` lookup would otherwise raise `KeyError` and the element would silently skip the bounds check), and `XLINK:HREF` / `Href` cannot slip the ends-in-`href` rule. **Duplicate-case attributes are refused outright on every element including the root `<svg>`** — an element carrying both `<rect X="3000" ... x="0" ...>` OR a root `<svg VIEWBOX="0 0 99999 99999" viewBox="0 0 1920 1080">` (or any other case-fold collision) is rejected because HTML5's first-wins case-folded attribute resolution would otherwise diverge from a naive last-wins dict-build, letting the validator see the safe lowercase value while the browser renders the unsafe uppercase one — and on the root that means the attacker can effectively redefine the SVG canvas to absorb any out-of-canvas geometry;
