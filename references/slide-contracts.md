...
- `cover`: emits a structural `line` (title divider, no slot), then `text` primitives for the required `title` and any present optional `subtitle` / `presenter` / `date`, then an `image_slot` for `accent` if the slide_plan has an `accent` block whose `image_ref` is declared in `image_manifest`. Bounds come from the layout slot when set; otherwise from the deterministic fallback table in the script. The `business_review` cover layout declares bounds for every slot today, so the fallback table is a forward-compat safety net rather than the live source.
...
- `executive_summary`: required `text` for `title` and `summary`, then one bulleted `text` per entry in the optional `key_points` list block, distributed vertically inside the `key_points` slot. Every bulleted list emitter (`executive_summary.key_points`, `two_column.left_content` / `right_content`, `agenda.agenda_items`, `conclusion.call_to_action`) shares one helper: each item's height is `min(slot.h // count, LIST_ITEM_MAX_H)` so a short list (2 – 3 bullets) packs tight at the top of the slot rather than sprawling across the full slot height. Trailing whitespace below the last item is intentional. The `timeline` layout uses the same helper for vertical distribution but applies a different visual shape — see its bullet below.
- `key_message`: optional `text` for `title`, a decorative `shape` (rounded_rectangle, no slot) sized to the `message` slot bounds, a `text` primitive overlaying it carrying the callout content, and an optional `text` for `supporting_text`. The `message` slot is sized for a single-line callout (not a multi-line statement); widen the slot bounds and lower `supporting_text` if multi-line callouts become a recurring shape.
- `two_column`: required `text` for `title`, then per side an optional `text` heading plus one bulleted `text` per entry in the required content list, distributed vertically inside each column's content slot (same `LIST_ITEM_MAX_H` clamp as `executive_summary`).
- `kpi_dashboard`: emits a `text` primitive for `title`, then for each entry in `slide_plan.blocks[id="kpis"].content` one structural `shape` (rounded_rectangle, no slot — the per-tile card background) immediately followed by one `kpi` primitive layered on top at the same bounds (the editable label / value / optional delta). Tiles are sized and positioned by a centered-row formula inside the `kpis` slot bounds; the slot is sized to wrap the actual label + value (+ delta) lines rather than reserving a band well below the content. There is no single outer band shape — every visible card carries its own border, so the dashboard reads as a grid of cards.
- `timeline`: required `text` for `title`, then one structural vertical `line` (`timeline_axis`, no slot) anchored on the left side of the `timeline_items` slot whose height equals the per-item slice times the step count, then per entry one structural `shape` (filled ellipse marker, no slot) centered on the axis at the item's vertical midline AND one `text` primitive (slot-bound to `timeline_items`) carrying the step content offset to the right of the marker. The step text carries no bullet prefix — the marker replaces it. The `timeline.json` layout does not declare bounds today; both slots fall back to the generator's `LAYOUT_FALLBACK_BOUNDS`.
- `agenda`: required `text` for `title`, then one bulleted `text` per entry in the required `agenda_items` list, distributed vertically inside the slot's fallback bounds (same `LIST_ITEM_MAX_H` clamp as the other list emitters). The `agenda.json` layout does not declare bounds today; the generator's `LAYOUT_FALLBACK_BOUNDS` covers the gap. Unlike `timeline`, the agenda emitter draws no axis line and no per-item marker — the list is intentionally flat.
- `conclusion`: required `text` for `title`, optional `text` for `summary`, then one bulleted `text` per entry in the optional `call_to_action` list, distributed vertically inside the slot bounds (same `LIST_ITEM_MAX_H` clamp as the other list emitters).
...
- The scaffold's `scripts/validate_workspace.py` looks for per-slide plans under `<workspace>/slide_plans/*.json`. Coverage / orphan / duplicate-by-index detection in `check_slide_plan_coverage` still keys off the JSON `index` field (not the filename), so a mis-named file is not silently mis-resolved to the wrong deck_plan slide. **Stage-5 commits to the flat layout:** `scripts/init_slide_plans.py` writes per-slide plans as `<workspace>/slide_plans/<idx:02d>_<layout>.json` (zero-padded two-digit `index` + `layout` token from inside the JSON — same shape as the render-model canonical name). On top of the index-keyed coverage check, the workspace validator now enforces the canonical filename on disk via `check_slide_plan_filenames` / `slide_plan.filename` in `references/quality-gates.md`: every `slide_plans/*.json` file MUST be named `<index:02d>_<layout>.json` matching its own JSON `index` + `layout`. Misnamed files, filename/index mismatches, filename/layout mismatches, and a duplicate canonical + non-canonical pair for the same JSON index (canonical sibling passes, non-canonical sibling fails) all fail closed with a FAIL line naming the expected canonical filename. Schema-invalid slide_plans (missing `index`/`layout`, wrong types, empty layout) emit zero rows from this gate — `check_schemas` / `check_slide_plan_coverage` already surface those, so the maintainer sees the schema error once rather than twice.
- **Render model filenames are canonical.** Every `render_models/*.json` file must be named `<index:02d>_<layout>.json`, where `index` and `layout` come from inside the JSON (zero-padded two-digit index + the layout token verbatim — e.g. `02_agenda.json`, `09_two_column.json`, `20_conclusion.json`). The PPTX exporter (`scripts/export_pptx.py`) iterates `deck_plan.slides[]` in declared order and resolves each entry to its canonical render_model filename, so a mis-named `99_agenda.json` whose JSON `index` is 2 fails closed at the 1:1 coverage gate (the canonical `02_agenda.json` is missing on disk, the mis-named file is orphan) before any output is written. The workspace validator enforces this on disk (`check_render_model_filenames` / `render.filename` in `references/quality-gates.md`); the render-model generator always writes the canonical name. Schema-invalid files (no integer `index`, no string `layout`) are not double-flagged by this gate — `check_render_models` already rejects them. The Stage-5 helper writes per-slide plans using the same canonical shape; `init_slide_plans.py` is the producer-side commitment, and the workspace-validator filename gate for `slide_plans/*.json` is enforced by `check_slide_plan_filenames` / `slide_plan.filename` (see the prior bullet).
