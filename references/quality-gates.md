...
| `render.generator.supported_layouts` — `scripts/generate_render_models.py` produces output for every layout reachable from the controlled `text` / `line` / `shape` / `image_slot` / `kpi` primitive set (`cover`, `section_divider`, `executive_summary`, `key_message`, `two_column`, `kpi_dashboard`, `timeline`, `conclusion`); layouts mapped to the `table` or `chart_placeholder` primitive kinds (e.g. `comparison_table`) are reported `[SKIP] … not implemented` and are NOT counted as success | Render model | workspace (tempfixture) |
