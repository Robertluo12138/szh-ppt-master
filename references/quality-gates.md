...
| `render.generator.supported_layouts` — `scripts/generate_render_models.py` produces output for every layout reachable from the controlled `text` / `line` / `shape` / `image_slot` / `kpi` primitive set (`cover`, `section_divider`, `executive_summary`, `key_message`, `two_column`, `kpi_dashboard`, `timeline`, `agenda`, `conclusion`); layouts mapped to the `table` or `chart_placeholder` primitive kinds (e.g. `comparison_table`) are reported `[SKIP] … not implemented` and are NOT counted as success | Render model | workspace (tempfixture) |
| `render.coverage.no_partial_drift` — once a workspace ships any `render_models/*.json`, every deck_plan slide whose layout is in the render-model generator's `SUPPORTED_LAYOUTS` set must have a matching render_model on disk. Slides whose layout is NOT in that set (today only `comparison_table`) are explicitly allowed to have no render_model. Empty `render_models/` directories still mean "not yet generated" and do not trigger the gate. The check is adaptive — no example name, slide count, or template name is hardcoded — and imports `SUPPORTED_LAYOUTS` lazily from `scripts/generate_render_models.py` so the two stay in sync | Render model | workspace (`check_generator_render_model_coverage` + tempfixture) |
| `render.filename` — every `render_models/*.json` file is named `<index:02d>_<layout>.json` (zero-padded two-digit `index` + `layout` token, both read from inside the JSON — e.g. `02_agenda.json`, `09_two_column.json`). The PPTX exporter sorts `render_models/*.json` by file stem and treats that order as the deck's slide order, so a mis-named file would silently land at the wrong slide position. The gate is fail-closed with a FAIL line that names the expected canonical filename. Schema-invalid files (no integer `index`, no string `layout`) are not double-flagged here — `render.schema` already surfaces those. This naming rule is render-model-specific; the broader workspace path layout (per-slide directory vs flat `slide_plans/`, etc.) remains TODO | Render model | workspace (`check_render_model_filenames` + tempfixture `negative_render_model_filename_tempfixture_checks`) |
...
| `pptx.export.scope` — `scripts/export_pptx.py` writes one native editable `.pptx` from a workspace's `render_models/*.json` for the supported subset only: layouts `cover` / `kpi_dashboard` / `agenda` / `section_divider` / `executive_summary` / `key_message` / `two_column` / `timeline` / `conclusion`; primitive kinds `text` / `line` / `shape` / `image_slot` / `kpi`. The slide-body emitter is layout-agnostic — it walks each render_model's `primitives` list — so the allow-list is purely a contract gate. `comparison_table` is intentionally NOT in the allow-list because its primary content is the still-unsupported `table` primitive. The contract is all-or-nothing: a render_model whose layout is outside the supported set, an unsupported primitive (`table`, `chart_placeholder`), wrong output extension, malformed render_model, undeclared image_ref, or unsafe manifest path all fail closed with a per-slide error, abort the whole run, and write no partial `.pptx` | PPTX export | implemented for the expanded subset (exporter `scripts/export_pptx.py`; self-test exercises every fail-closed gate plus a happy-path positive and an expanded-layout positive (`two_column`) that re-checks the produced `.pptx` against the validator) |
...
| `pptx.layouts.scope` — exported slides use only the layouts on `scripts/export_pptx.py`'s `SUPPORTED_LAYOUTS` allow-list (`cover`, `kpi_dashboard`, `agenda`, `section_divider`, `executive_summary`, `key_message`, `two_column`, `timeline`, `conclusion`) | PPTX export | exporter-enforced today; validator-side check is TODO |
