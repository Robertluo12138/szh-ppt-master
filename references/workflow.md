...
Stage 7 is **partially implemented**: `scripts/generate_render_models.py` produces deterministic render models for every layout reachable from the controlled `text` / `line` / `shape` / `image_slot` / `kpi` primitive set — `cover`, `section_divider`, `executive_summary`, `key_message`, `two_column`, `kpi_dashboard`, `timeline`, `conclusion`. Layouts whose required slot maps to the `table` or `chart_placeholder` primitive_kind (`comparison_table`, anything chart-bearing, and `agenda` until it is wired up) are listed as `[SKIP] … not implemented` by the script and are **not** counted as success. Slides for unsupported layouts have no `render_model.json` and cannot proceed to stages 8 / 10 until the generator is extended. The script is fail-closed on missing / malformed inputs, unsafe `deck_plan.template`, unknown `image_ref`, malformed kpi entries, missing required slide_plan block, list slots with empty / non-string content, schema-invalid output, or workspace cross-check failure.
