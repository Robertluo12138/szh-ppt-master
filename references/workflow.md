...
The render-model output is canonically named `<index:02d>_<layout>.json` (zero-padded two-digit index + layout token from the JSON — e.g. `02_agenda.json`, `09_two_column.json`). The workspace validator's `check_render_model_filenames` gate (`render.filename` in `references/quality-gates.md`) enforces this on disk, because the stage-10 PPTX exporter sorts `render_models/*.json` by file stem and treats that order as the deck's slide order — a mis-named file would silently land at the wrong slide position. This naming rule is render-model-specific; the broader workspace path layout for `slide_plans/` and friends remains TODO (see `references/slide-contracts.md`).
...
Stage 10 (PPTX export) is **partially implemented**. `scripts/export_pptx.py` is a deterministic, stdlib-only OOXML exporter that consumes a workspace's `render_models/*.json` directly — never `slide_plan.json`, never `svg_previews/*.svg`, never any rasterized slide image. The expanded native editable subset it currently supports is:
