...
Scaffold + expanded vertical slice. The pipeline is defined, the on-disk contracts are sketched, the render-model stage produces output for the controlled `text` / `line` / `shape` / `image_slot` / `kpi` primitive set across `cover`, `section_divider`, `executive_summary`, `key_message`, `two_column`, `kpi_dashboard`, `timeline`, and `conclusion`; an SVG preview stage renders those render-models into per-slide `*.svg` files under `svg_previews/`; and an **expanded native editable PPTX export stage** (`scripts/export_pptx.py`) now covers `cover`, `kpi_dashboard`, `agenda`, `section_divider`, `executive_summary`, `key_message`, `two_column`, `timeline`, and `conclusion` (primitives `text` / `line` / `shape` / `image_slot` / `kpi`; `image_slot` is rendered as a native placeholder shape — media embedding is intentionally TODO). The exporter's slide-body emitter is layout-agnostic — it walks each render_model's `primitives` list — so widening the layout allow-list does not change how any single shape is rendered. Layouts that map to the `table` or `chart_placeholder` primitive kinds (e.g. `comparison_table`) remain unimplemented and are reported as `[SKIP] ... not implemented` by the render-model generator and fail closed at the exporter's layout gate. Charting, visual regression, image generation, full editability inventory, theme palette mapping, validator-side layout/primitive-scope inspection of the produced PPTX, media inventory, and CLI integration are not implemented yet. See per-file TODOs and `references/`.
...
- Render-model generation for layouts mapped to the `table` or `chart_placeholder` primitive kinds. `scripts/generate_render_models.py` covers `cover`, `section_divider`, `executive_summary`, `key_message`, `two_column`, `kpi_dashboard`, `timeline`, `conclusion`; layouts like `comparison_table` and any chart-bearing layout (and `agenda` until it is wired up) are listed as `[SKIP] … not implemented` and are **not** claimed as success.
