...
- `scripts/validate_pptx_contract.py` — stdlib-only, fail-closed **contract / skeleton** validator for the future PPTX export stage. PPTX export itself is NOT implemented; this script does not generate a `*.pptx` and is not proof that the exporter works. In skeleton mode (no `--pptx`) it reports the TODO surface only (editability, no all-image slides, relationship allow-list, embedded-only media, theme palette mapping, determinism, layout scope `cover` / `kpi_dashboard`, primitive scope `text` / `line` / `shape` / `image_slot` / `kpi`; `table` and `chart_placeholder` must fail closed in the future exporter). With `--pptx <path>` it runs the basic OOXML container checks (file exists, `.pptx` extension, readable ZIP, required package entries `[Content_Types].xml` / `_rels/.rels` / `ppt/presentation.xml`). `--self-test` exercises tempfixture negatives (missing file, wrong extension, non-zip content, empty ZIP, ZIP missing `ppt/presentation.xml`) plus a minimal-valid-container positive. See `references/pptx-conversion-rules.md` and `references/quality-gates.md` (`pptx.contract.skeleton`).
...
- PPTX conversion (no exporter exists yet). `scripts/validate_pptx_contract.py` is a contract / skeleton validator only — it locks the OOXML container surface and the TODO surface for the future exporter, but it does NOT generate a `*.pptx` and a passing run is NOT proof that export works. Editability inspection, relationship allow-list, embedded-only media, theme palette mapping, determinism, and full security scan all remain TODO inside that validator.
...
# PPTX export contract / skeleton validator. PPTX export is NOT
# implemented; this script does not generate a .pptx. In skeleton
# mode it reports the contract / TODO surface only. With --pptx it
# runs the basic OOXML container checks. --self-test runs the
# tempfixture negatives plus a minimal-valid-container positive.
python3 scripts/validate_pptx_contract.py
python3 scripts/validate_pptx_contract.py --self-test
```

All commands exit non-zero if any check disagrees, including built-in negative cases (e.g. unsafe `http://` / `s3://` / `data:` paths must be rejected, dropping a required layout slot must be detected, an orphan slide_plan must be detected, a render_model that uses an unsupported `kind` must be rejected, the generator must fail closed on an unknown `image_ref`, an SVG preview missing for a render_model or carrying a `<foreignObject>` / unsafe `href` / `<text>` anchor outside the canvas must fail). SVG-preview generation and validation are implemented for the supported primitive subset (`text`, `line`, `shape`, `image_slot`, `kpi`) and only run against render_models the generator produces today (`cover`, `kpi_dashboard`); slides on every other layout have no render_model and no SVG preview yet. PPTX export is NOT implemented; `validate_pptx_contract.py` is a contract / skeleton validator only — a passing run reports the TODO surface and (when `--pptx` is supplied) confirms only the basic OOXML container shape, never editability or relationship safety. SVG repair, the real PPTX exporter, security scan, visual regression, and SVG / render-model coverage for the remaining layouts and primitive kinds remain TODO until their scripts exist.
