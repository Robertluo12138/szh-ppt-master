...
- Full PPTX generation. The minimal vertical slice (`scripts/export_pptx.py`) covers the same supported subset as the render-model generator and the SVG renderer: layouts `cover` / `kpi_dashboard`; primitive kinds `text` / `line` / `shape` / `image_slot` / `kpi`. Out of scope (fail-closed in the exporter, TODO in the deeper validator):
  - PPTX emission for `table` and `chart_placeholder` primitives;
  - PPTX emission for layouts other than `cover` / `kpi_dashboard`;
  ...
  - validator-side full-inventory editability, relationship allow-list, media inventory, determinism, layout-scope, and primitive-scope checks inside `scripts/validate_pptx_contract.py` (today the validator runs container checks plus minimal-evidence checks for safety / editability surface area, both reported as MINIMAL EVIDENCE only).
...
# Minimal native editable PPTX export. Reads
# <workspace>/render_models/*.json (sorted by file stem) and writes
# a deterministic .pptx covering the supported subset (cover and
# kpi_dashboard; primitives text / line / shape / image_slot / kpi).
# `table`, `chart_placeholder`, every other layout, wrong --output
# extension, malformed render_model, undeclared image_ref, and
# unsafe manifest local_path all fail closed. --self-test exercises
# every gate against a synthetic workspace under
...
# external rels, no file:// rels, no macros / OLE / ActiveX parts,
# at least one editable text run, no all-image slide). --self-test
# exercises every gate against tempfixture negatives plus a
# minimal-valid-container positive and a minimal-editable PPTX
# positive.
...
All commands exit non-zero if any check disagrees, including built-in negative cases (e.g. unsafe `http://` / `s3://` / `data:` paths must be rejected, dropping a required layout slot must be detected, an orphan slide_plan must be detected, a render_model that uses an unsupported `kind` must be rejected, the generator must fail closed on an unknown `image_ref`, an SVG preview missing for a render_model or carrying a `<foreignObject>` / unsafe `href` / `<text>` anchor outside the canvas must fail, an `.pptx` carrying an external relationship / `file://` Target / vbaProject / OLE / ActiveX part / all-image slide / no editable text must be rejected). SVG-preview generation and validation are implemented for the supported primitive subset (`text`, `line`, `shape`, `image_slot`, `kpi`) and only run against render_models the generator produces today (`cover`, `kpi_dashboard`); slides on every other layout have no render_model and no SVG preview yet. The PPTX exporter covers the same supported subset and emits native editable shapes (text frames, connectors, preset shapes); `image_slot` is rendered as a native placeholder rectangle with alt_text (media embedding is TODO). `validate_pptx_contract.py --pptx` reports a passing run as `OK (container + minimal-evidence)` and explicitly names the full-inventory editability, relationship allow-list, media inventory, theme palette mapping, determinism, layout-scope, and primitive-scope checks that remain TODO. SVG repair, full PPTX coverage, security scan, visual regression, and SVG / render-model coverage for the remaining layouts and primitive kinds remain TODO until their scripts exist.
