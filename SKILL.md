...
# text); stages 5-6 (slide_plans / image_manifest) remain
# agent-driven before run_pipeline.py can take over. --source must be a regular file (not
...
# Stage 4 has narrow contract support via init_design_system.py
# (see below); stages 5-6 (slide_plans / image_manifest) remain
# agent-driven before run_pipeline.py can take over. Fail-closed gates:
...
# declared in deck_brief.source_refs — so source_manifest.source.id
# (which the bridge requires to be in deck_brief.source_refs)
# remains part of the deck-level source bundle every slide draws
# from. This is a deck-level traceability claim, NOT a per-slide
# one: when deck_brief.source_refs declares multiple ids, a slide
# may legitimately pick a non-manifest id from that set).
# Stage 4 has narrow contract support via init_design_system.py
# (see below); stages 5-6 (slide_plans / image_manifest) remain
# agent-driven before run_pipeline.py can take over. Fail-closed gates:
# --workspace must be an existing directory whose string form is not
